...
<script>
    ...
    export default {
        ...
        mounted:function(){
            ...
            this.$axios.get('/api/hangzhou')
            .then(function(data) {
                console.log(data)
                var points = [].concat.apply([], data.data.data.map(function(track) {
                    return track.map(function(seg) {
                        return seg.coord.concat([1]);
                    });
                }));
                _this.chartBar.setOption({
                    animation: false,
                    bmap: {
                ...
                // 添加百度地图插件
                var bmap = _this.chartBar.getModel().getComponent('bmap').getBMap();
